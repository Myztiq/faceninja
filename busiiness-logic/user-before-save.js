...
var urlConfig = {
  ...
  // OAuth1.0a and 2.0.
  authenticate: {
    facebook: 'https://www.facebook.com/dialog/oauth?display=popup',
  ...
  if (null != provider) {
    modules.collectionAccess.collection('oauth').findOne({ _id: provider }, function (err, doc) {
      if (err) {// Request failed.
        ...
        // OAuth1.0a providers support create and login through this proxy.
        if (oauth1 && -1 !== ['create', 'login'].indexOf(step)) {
          loginOrCreate(step, doc, request, response, modules);
        }
        // Request a token.
 ...
 *
 * @param {string} method Method, either "login" or "create".
 * @param {Object} doc OAuth provider document.
 * @param {Object} request Kinvey request object.
 * @param {Object} response Kinvey response object.
 * @param {Object} modules Available JavaScript libraries.
 */
function loginOrCreate(method, doc, request, response, modules) {
  ...
  // Forward request to login or create endpoint.
  modules.request.post({
    uri: 'https://' + encodeURIComponent(request.headers.host) + '/user/' + encodeURIComponent(request.username) + '/' + ('login' === method ? 'login' : ''),
    headers: {
 ...
 * @param {Object} modules Available JavaScript libraries.
 */
function requestOAuth1Token(doc, request, response, modules) {
  // Validate request body.
  var provider = doc._id;
 ...
 * @param {Object} modules Available JavaScript libraries.
 */
function requestOAuth2Token(doc, request, response, modules) {
  // Validate request body.
  var provider = doc._id;
  if (!request.body.redirect) {
    response.body = {
 ...
 * @param {Object} modules Available JavaScript libraries.
 */
function verifyToken(doc, request, response, modules) {
  // Validate request body.
  var provider = doc._id;
